...
def remap_state_dict_hf_opt(state_dict, config):
    ...
    for l in range(config.num_hidden_layers):
        Wq = state_dict.pop(f'model.decoder.layers.{l}.self_attn.q_proj.weight')
        Wk = state_dict.pop(f'model.decoder.layers.{l}.self_attn.k_proj.weight')
        Wv = state_dict.pop(f'model.decoder.layers.{l}.self_attn.v_proj.weight')
        bq = state_dict.pop(f'model.decoder.layers.{l}.self_attn.q_proj.bias')
        bk = state_dict.pop(f'model.decoder.layers.{l}.self_attn.k_proj.bias')
        bv = state_dict.pop(f'model.decoder.layers.{l}.self_attn.v_proj.bias')
        state_dict[f'model.decoder.layers.{l}.self_attn.Wqkv.weight'] = torch.cat([Wq, Wk, Wv], dim=0)
        state_dict[f'model.decoder.layers.{l}.self_attn.Wqkv.bias'] = torch.cat([bq, bk, bv], dim=0)
    def key_mapping_attn(key):
        return re.sub(r'^model.decoder.layers.(\d+).self_attn.out_proj.',
                      r'model.decoder.layers.\1.self_attn.out_proj.', key)
